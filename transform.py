...
def merge_tables(tables):
    ...
    for table in tables:
        for head in headers(conn, table):
            ...
            if head not in header:
                header.append(head)
    h_str = ", ".join(header)
    sql = "create table {} ({})".format(key, h_str)
    conn.execute(sql)
    hlist = []
    for table in tables:
        l = []
    ...
    sql = "insert into {} ({}) select {} from {} union select {} from {}".format(key, h_str, hlist[0][1], hlist[0][0], hlist[1][1], hlist[1][0])
    conn.execute(sql)
    for table in tables:
    ...
    for req_f in req:
        if req_f not in fields_str:
            if req_f in ifields.unique[filetypeout]:
                ...
                if len(key) > 1:
                    fields_str += ', ' + ' || '.join('ifnull(' + table + '.' + piece + ', "")' for piece in key) + ' as ' + req_f
    table_join = ''
    ...
    for i in all_fields:
        if i in fields_req:
            ...
            if i == 'specimen_type':
                ...
                print pandas.notnull(w2[i]), w2[i]
...
if __name__ == "__main__":
    ...
    with sqlite3.connect(':memory:') as conn:
        for file in listdir(p.dir):
            load(conn, file, p.dir.strip('/'))
        # check to see if there are multiple tables of the same type
        num_filetypes = {}
                ...
                for filealias in filetype:
                    if filealias in table:
                        if filealias in num_filetypes:
                            num_filetypes[filetype[0]].append(table)
                        else:
                            num_filetypes[filetype[0]] = [table]
        for key in num_filetypes:
            if len(num_filetypes[key]) > 1:
                merge_tables(num_filetypes[key])
                num_filetypes[key] = [key]
        get_donor(conn, p.dir.strip('/'), num_filetypes)
        get_spec(conn, p.dir.strip('/'), num_filetypes)
        get_samp(conn, p.dir.strip('/'), num_filetypes)